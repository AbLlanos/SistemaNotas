...
public class EducacionInicialController {
    ...
    public String vistaAdminEducaionInicial(
        ...
        String nivelFiltro = "educacioninicial";
        ...
        List<String> cedulasFiltradas = estudianteService.listarVisibles().stream()
                .filter(e -> {
                    if (e.getNivelEducativo() == null || e.getNivelEducativo().getNombre() == null) {
                        return false;
                    }
        ...
        if (!mostrarOcultos) {
            notas = notas.stream()
                    .filter(n -> cedulasFiltradas.contains(n.getCedulaEstudiante()))
                    .toList();
        }
    ...
    @GetMapping("/notas/nuevoEducacionInicial")
    public String mostrarFormularioNuevaNotaInicial(Model model) {
    ...
    @GetMapping("/periodo/{nombrePeriodo}/cursosEducacionInicial")
    @ResponseBody
    public List<CursoDTO> listarCursosPorPeriodoFiltrados(@PathVariable String nombrePeriodo) {
        ...
        List<CursoDTO> cursosFiltrados = cursos.stream()
                .filter(curso -> curso.getNivelEducativo() != null &&
                        //MODIFICAR
                        "Educacion Inicial".equals(curso.getNivelEducativo().getNombre()))
